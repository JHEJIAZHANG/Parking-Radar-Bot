...
if auth_response.status_code == 200:
    ...
    print("✅ 成功取得通行證！準備開始掃描【全台高速公路休息站】...\n")
    ...
    while True:
        url = f"https://tdx.transportdata.tw/api/basic/v1/Parking/OffStreet/CarPark/Road/Freeway/ServiceArea?$top={top}&$skip={skip}&$format=JSON"
        ...
        if response.status_code == 200:
            ...
            for park in park_list:
                try:
                    ...
                    fare = park.get("FareDescription", "免費") # 休息站通常免費
                    address = park.get("Address", "無地址資訊")
                    city = park.get("City", "國道") 
    ...
    with open(filename, mode="w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["縣市/國道", "停車場ID", "停車場名稱", "緯度", "經度", "費率資訊", "電動車充電樁", "地址"]) 
        writer.writerows(all_clean_data) 

    print(f"🎉 任務圓滿結束！總共收集了 {len(all_clean_data)} 筆休息站資料，檔案已存為 {filename}")
