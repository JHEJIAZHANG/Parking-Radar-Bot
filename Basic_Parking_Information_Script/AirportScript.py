...
if auth_response.status_code == 200:
    ...
    for auth in authorities:
        print(f"🚀 開始抓取航空單位：【{auth}】")
        skip = 0            
        
        while True:
            # 網址替換為 Air/Airport 跟 authority
            url = f"https://tdx.transportdata.tw/api/basic/v1/Parking/OffStreet/CarPark/Air/Airport/{auth}?$top={top}&$skip={skip}&$format=JSON"
            ...
            if response.status_code == 200:
                ...
                for park in park_list:
                    try:
                        ...
                        # 把「航空單位」存進第一欄位
                        all_clean_data.append([auth, park_id, name, lat, lng, fare, ev_status, address])
                    except Exception:
                        continue 
                ...
                print(f"❌ 抓取 {auth} 發生錯誤，狀態碼：{response.status_code}，跳過此單位。")
                break
    ...
    with open(filename, mode="w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["航空單位", "停車場ID", "停車場名稱", "緯度", "經度", "費率資訊", "電動車充電樁", "地址"]) 
        writer.writerows(all_clean_data) 

    print(f"🎉 任務圓滿結束！總共收集了 {len(all_clean_data)} 筆航空站停車資料，檔案已存為 {filename}")
