...
if auth_response.status_code == 200:
    ...
    print("✅ 成功取得通行證！準備開始掃描【全台軌道車站停車場】...\n")
    ...
    for operator in operators:
        print(f"🚀 開始抓取業者：【{operator}】")
        skip = 0            
        page = 1 
        
        while True:
            # 網址替換為 Rail 跟 operator
            url = f"https://tdx.transportdata.tw/api/basic/v1/Parking/OffStreet/CarPark/Rail/Station/{operator}?$top={top}&$skip={skip}&$format=JSON"
            ...
            if response.status_code == 200:
                ...
                for park in park_list:
                    try:
                        ...
                        # 這裡我們把「縣市」欄位替換成「軌道業者」，方便以後辨識
                        all_clean_data.append([operator, park_id, name, lat, lng, fare, ev_status, address])
                    except Exception:
                        continue 
                ...
                print(f"❌ 抓取 {operator} 發生錯誤，狀態碼：{response.status_code}，跳過此業者。")
                break
    ...
    with open(filename, mode="w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["軌道業者", "停車場ID", "停車場名稱", "緯度", "經度", "費率資訊", "電動車充電樁", "地址"]) 
        writer.writerows(all_clean_data) 

    print(f"🎉 任務圓滿結束！總共收集了 {len(all_clean_data)} 筆軌道車站停車資料，檔案已存為 {filename}")
